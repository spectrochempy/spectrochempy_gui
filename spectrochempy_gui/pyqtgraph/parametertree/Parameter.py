    ...
    **Signals:**
    ...
    def restoreState(self, state, recursive=True, addChildren=True, removeChildren=True, blockSignals=True):
        ...
        try:
            ...
            foundChilds = set()
            # scp.debug_("==============", self.name())
            for ch in childState:
                name = ch['name']
                typ = ch.get('type', None)
                # scp.debug_(f'child: {self.name()}.{name}, {typ}')
                ...
                for i, ch2 in enumerate(self.childs[ptr:]):
                    if ch2.name() != name: # or not ch2.isType(typ):
                        continue
                    gotChild = True
                    # scp.debug_(f">   {ch2.name()} {ch2.type()} >>> found it")
                    if i != 0:  ## move parameter to next position
                        #self.removeChild(ch2)
                        self.insertChild(ptr, ch2)
                        # # scp.debug_("  moved to position", ptr)
                    ch2.clearChildren()
                    ch2.restoreState(ch, recursive=recursive, addChildren=addChildren, removeChildren=removeChildren, blockSignals=blockSignals)
                    foundChilds.add(ch2)
                ...
                if not gotChild:
                    if not addChildren:
                        # scp.debug_("  ignored child")
                        continue
                    # scp.debug_(">    created new")
                    ch2 = Parameter.create(**ch)
                    self.insertChild(ptr, ch2)
                    if ch.get('children', None):
                        ch2.restoreState(ch, recursive=recursive, addChildren=addChildren, removeChildren=removeChildren, blockSignals=blockSignals)
                    foundChilds.add(ch2)
